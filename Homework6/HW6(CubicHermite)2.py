...
def CubicHermite(pts, time_passed, t, h_list, b_list, b2_list):
    # pt0 = pts[-4]
    # pt1 = pts[-3]
    # pt2 = pts[-2]
    # pt3 = pts[-1]
    ...
    # CFS(pt1, pt2, 'GRAY')
    # if t <= 1:
    #     h = (1-t)**3*pt0 + 3*t*(1-t)**2*pt1 + 3*t**2*(1-t)*pt2 + t**3*pt3
    #     h_list.append(h)
    #     for h in h_list:
    #         drawPoint(h, 'RED', 2)
    # else:
    #     for h in h_list:
    #         drawPoint(h, 'RED', 2)
    # _, b_list, _ = Bezier2([pt0, pt1], time_passed, t, b_list, 'GREEN', 1)
    # _, b2_list, _ = Bezier2([pt2, pt3], time_passed, t, b2_list, 'GREEN', 1)
    # t_img = small_font.render('t = {0}'.format(t), True, BLACK)
    # screen.blit(t_img, (10, 40))
    # return t, h_list, b_list, b2_list
    ...
    h_list.append(h)
    drawPoint(h, 'RED', 5)
    return u, h_list, b_list, b2_list

# HW7 : implement cubic B-spline with HW2
def CardinalSpline(pts, time_passed, t, h_list, b_list, b2_list):
    pt0 = pts[-4]
    pt1 = pts[-3]
    pt2 = pts[-2]
    pt3 = pts[-1]
...
################################################ Display ##################################################
    ...
    if len(pts) >= MinPoints:
        ...
        # Hermite(pts)
        t, h_list, b_list, b2_list = CubicHermite(pts, time_passed, t, h_list, b_list, b2_list)   # 4

        # t, h_list, b_list, b2_list = CardinalSpline(pts, time_passed, t, h_list, b_list, b2_list) # 4
