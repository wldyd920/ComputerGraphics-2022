...
def Hermite(pts):
    ...
    for i in range(1000):
        ...
        h = np.matmul(sth, g_mat)
        drawPoint(h, 'GRAY', 1)

def HermiteCurve(pts, time_passed, t, h_list):
    Hermite(pts)
    p12 = [pts[0], pts[2]]
    r12 = [pts[1], pts[3]]
    g_mat = []
...
def HermiteSpline(pts):
    ...
    for t in np.arange(0, 1, 0.005):
        ...
        for i in np.arange(0, n-1, 1):
            if n > 2:
                if i == 0:
                    val_pt2 = pts_arr[i + 2] - pts_arr[i]
                    val1 = np.zeros(2, dtype=np.float32)
                    val2 = np.dot(const, val_pt2)
                elif i > 0 and i < n-2:
                    val_pt1 = pts_arr[i + 1] - pts_arr[i - 1]
                    val_pt2 = pts_arr[i + 2] - pts_arr[i]
                    val1 = np.dot(const,val_pt1)
                    val2 = np.dot(const,val_pt2)
                else:
                    val_pt1 = pts_arr[i + 1] - pts_arr[i - 1]
                    val1 = np.dot(const, val_pt1)
                    val2 = np.zeros(2, dtype=np.float32)
            else:
                val1 = np.zeros(2, dtype=np.float32)
                val2 = np.zeros(2, dtype=np.float32)

            ch = np.dot(f1,pts[i]) + np.dot(f2,pts[i+1]) + np.dot(f3,val1) + np.dot(f4,val2)
            ch = ch.astype(int)
            drawPoint(ch, color=RED, thick=1)
...
################################################ Display ##################################################
    ...
    if len(pts) >= MinPoints:
        ...
        # Hermite(pts)
        # t, h_list = HermiteCurve(pts, time_passed, t, h_list)      # 4
        HermiteSpline(pts)
