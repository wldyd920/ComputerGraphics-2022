...
def B_Spline(pts):
    ...
    for i in range(n):
        if i == 0:
            A[i][i + 1] = 1
            f[i] = np.dot(3, (pts_arr[i+1]-pts_arr[i]))
        elif i > 0 and i < n-1:
            A[i][i - 1] = 1
            A[i][i + 1] = 1
            f[i] = np.dot(3, (pts_arr[i + 1] - pts_arr[i-1]))
        else:
            A[i][i - 1] = 1
            f[i] = np.dot(3, (pts_arr[i] - pts_arr[i - 1]))

        # Draw Point
        # pygame.draw.rect(screen, GREEN, (pts[i][0] - margin, pts[i][1] - margin, 2 * margin, 2 * margin), 5)
    ...
    for t in np.arange(0, 1, 0.005):
        for i in range(n-1):
            a = pts_arr[i]
            b = M[i]
            c = np.dot(3, (pts_arr[i+1] - pts_arr[i])) - np.dot(2, M[i]) - M[i+1]
            d = np.dot(2, (pts_arr[i] - pts_arr[i+1])) + M[i] + M[i+1]
